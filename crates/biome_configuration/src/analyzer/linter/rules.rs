...
pub struct Nursery {
    ...
    #[doc = "Disallow usage of dependency arrays in createEffect and createMemo."]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub no_react_deps: Option<RuleConfiguration<biome_js_analyze::options::NoReactDeps>>,
...
impl RuleGroupExt for Nursery {
    ...
    fn get_enabled_rules(&self) -> FxHashSet<RuleFilter<'static>> {
        ...
        if let Some(rule) = self.no_react_deps.as_ref() {
            if rule.is_enabled() {
                index_set.insert(RuleFilter::Rule(Self::GROUP_NAME, Self::GROUP_RULES[29]));
            }
        }
